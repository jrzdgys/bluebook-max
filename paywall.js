...
var AUTH_CODES_URL = 'https://jrzdgys.github.io/bluebook-max/auth-codes.json?v=' + Date.now();
var AUTH_KEY = 'bbm_auth_token';
var FP_CACHE_KEY = 'bbm_fp_cache';
var EXPIRES_KEY = 'bbm_expires_at';
var FALLBACK_SECRET = 'bbm-fallback-v1-client-key-2026';
  ...
  return new Promise(function(resolve) {
    try {
      if (!crypto || !crypto.subtle || !crypto.subtle.importKey || !crypto.subtle.sign) {
        resolve(fallbackHmac(message, secret)); return;
      }
      ...
      }).catch(function() { resolve(fallbackHmac(message, secret)); });
    } catch(e) { resolve(fallbackHmac(message, secret)); }
  });
}

function fallbackHmac(message, secret) {
  var h = 0; var s = secret + '|' + message;
  for (var i = 0; i < s.length; i++) { h = ((h << 5) - h) + s.charCodeAt(i); h |= 0; }
...
function loadAuthCodes() {
  ...
  return fetch(AUTH_CODES_URL).then(function(r) { return r.json(); }).then(function(data) {
    _authCodesCache = data; return data;
...
function _injectAuthCSS() {
  ...
  s.textContent = '.auth-overlay{position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,.5);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;animation:pwFade .3s ease}' +
    ...
    '.auth-input{width:100%;padding:14px 18px;border:2px solid #E5E5EA;border-radius:14px;font-size:18px;text-align:center;letter-spacing:2px;font-family:+SF+Monospace,+Menlo,monospace;outline:none;transition:border-color .2s;box-sizing:border-box}' +
...
/** Build modal HTML using DOM methods to avoid quote escaping issues */
